...
y = data["target"]
x_train, x_test, y_train, y_test = train_test_split(X_normalized.values, y.values, test_size=0.1)
```
...
```{python}
(final_x, x_valid) = x_train[1400:], x_train[:1400]
(final_y, y_valid) = y_train[1400:], y_train[:1400]
```

```{python}
model = Sequential()
model.add(Dense(128, input_dim=final_x.shape[1], activation='relu'))
...
```{python}
result = model.fit(final_x, final_y, epochs=25, batch_size=64, validation_data=(x_valid, y_valid), verbose=1)
```
